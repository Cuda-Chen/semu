// This file was automatically generated by Makefile at
// https://github.com/cnlohr/cnfa Generated from files git hash
// a24b0f84ff7bf634217b21347c299bade7578d05 on Fri May 17 12:13:03 AM PDT 2024
// (This is not the git hash of this file) Copyright <>< 2010-2020 Charles Lohr
// (And other authors as cited) CNFA is licensed under the MIT/x11, ColorChord
...
// this #define is per-platform.  For instance on Linux, you have ALSA, Pulse
// and null
...
// NOTE: Some drivers have synchronous duplex mode, other drivers will use two
// different callbacks.  If ether is unavailable, it will be NULL. I.e. if `out`
// is null, only use in to read.  If in is null, only place samples in out.
typedef void (*CNFACBType)(struct CNFADriver *sd,
                           short *out,
                           short *in,
                           int framesp,
                           int framesr);

typedef void *(CNFAInitFn) (CNFACBType cb,
...
//
//  our_source_name is an optional argument, but on some platforms controls the
//  name of your endpoint. reqSPSPlay = 44100 is guaranteed on many platforms.
//  reqSPSRec = 44100 is guaranteed on many platforms.
//    NOTE: Some platforms do not allow SPS play and REC to deviate from each
//    other.
//  reqChannelsRec = 1 or 2 guaranteed on many platforms.
//  reqChannelsPlay = 1 or 2 guaranteedon many platforms. NOTE: Some systems
//  require ChannelsPlay == ChannelsRec! sugBufferSize = No promises.
//  outputSelect = No standardization, NULL is OK for default.
//  inputSelect = No standardization, NULL is OK for default.

DllExport struct CNFADriver *CNFAInit(const char *driver_name,
                                      const char *your_name,
                                      CNFACBType cb,
                                      int reqSPSPlay,
                                      int reqSPSRec,
...
DllExport int CNFAState(
    struct CNFADriver *cnfaobject);  // returns bitmask.  1 if mic recording, 2
                                     // if play back running, 3 if both running.
DllExport void CNFAClose(struct CNFADriver *cnfaobject);


// Called by various sound drivers.  Notice priority must be greater than 0.
// Priority of 0 or less will not register. This is an internal function.
// Applications shouldnot call it.
...
#define REGISTER_CNFA(cnfadriver, priority, name, function) \
    void REGISTER##cnfadriver()                             \
    {                                                       \
        RegCNFADriver(priority, name, function);            \
    }
#else
#define REGISTER_CNFA(cnfadriver, priority, name, function)  \
    void __attribute__((constructor)) REGISTER##cnfadriver() \
    {                                                        \
        RegCNFADriver(priority, name, function);             \
    }
#endif
    ...
    defined(__MINGW64__) || defined(__TOS_WIN__)
...
// CNFA is licensed under the MIT/x11, ColorChord or NewBSD Licenses. You
// choose.
...
void RegCNFADriver(int priority, const char *name, CNFAInitFn *fn)
{
    ...
    for (j = MAX_CNFA_DRIVERS - 1; j >= 0; j--) {
        ...
        if (j > 0 &&
            (!CNFADrivers[j - 1] || CNFADriverPriorities[j - 1] < priority)) {
                            ...
                            void *opaque)
{
    ...
    if (driver_name == 0 || strlen(driver_name) == 0) {
        ...
        if (bestinit) {
            ret = (struct CNFADriver *) bestinit(
                cb, your_name, reqSPSPlay, reqSPSRec, reqChannelsPlay,
                reqChannelsRec, sugBufferSize, outputSelect, inputSelect,
                opaque);
        }
        if (ret) {
            return ret;
        ...
        for (i = 0; i < MAX_CNFA_DRIVERS; i++) {
            ...
            if (strcmp(CNFADriverNames[i], driver_name) == 0) {
                return (struct CNFADriver *) CNFADrivers[i](
                    cb, your_name, reqSPSPlay, reqSPSRec, reqChannelsPlay,
                    reqChannelsRec, sugBufferSize, outputSelect, inputSelect,
                    opaque);
            }
...
void *InitCNFANull(CNFACBType cb,
                   const char *your_name,
                   int reqSPSPlay,
                   int reqSPSRec,
                   int reqChannelsPlay,
                   ...
                   void *opaque)
{
    struct CNFADriverNull *r =
        (struct CNFADriverNull *) malloc(sizeof(struct CNFADriverNull));
...
#endif
#elif CNFA_ANDROID
// Copyright 2019-2020 <>< Charles Lohr under the ColorChord License, MIT/x11
// license or NewBSD Licenses.
...
void bqRecorderCallback(SLAndroidSimpleBufferQueueItf bq, void *context)
{
    ...
    r->callback((struct CNFADriver *) r, 0, r->recorderBuffer, 0,
                r->buffsz / (sizeof(short) * r->channelsRec));
    (*r->recorderBufferQueue)
        ->Enqueue(
            r->recorderBufferQueue, r->recorderBuffer,
            r->recorderBufferSizeBytes / (r->channelsRec * sizeof(short)));
}

void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context)
{
    struct CNFADriverAndroid *r = (struct CNFADriverAndroid *) context;
    r->callback((struct CNFADriver *) r, r->playerBuffer, 0,
                r->buffsz / (sizeof(short) * r->channelsPlay), 0);
    (*r->playerBufferQueue)
        ->Enqueue(r->playerBufferQueue, r->playerBuffer,
                  r->playerBufferSizeBytes / (r->channelsPlay * sizeof(short)));
}
...
static struct CNFADriverAndroid *InitAndroidDriver(struct CNFADriverAndroid *r)
{
    ...
    result =
        (*r->engineObject)
            ->GetInterface(r->engineObject, SL_IID_ENGINE, &r->engineEngine);
    assert(SL_RESULT_SUCCESS == result);
    (void) result;
    ...
    if (r->channelsPlay) {
        ...
        SLDataLocator_AndroidSimpleBufferQueue loc_bq_play = {
        ...
        result = (*r->engineEngine)
                     ->CreateOutputMix(r->engineEngine, &r->outputMixObject, 0,
                                       ids, req);
        result = (*r->outputMixObject)
                     ->Realize(r->outputMixObject, SL_BOOLEAN_FALSE);
        ...
        // create audio player
        result = (*r->engineEngine)
                     ->CreateAudioPlayer(r->engineEngine, &r->playerObject,
                                         &source, &sink, 1, id, req);
        ...
        result =
            (*r->playerObject)
                ->GetInterface(r->playerObject, SL_IID_PLAY, &r->playerPlay);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        ...
        result =
            (*r->playerObject)
                ->GetInterface(r->playerObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                               &r->playerBufferQueue);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        ...
        result =
            (*r->playerBufferQueue)
                ->RegisterCallback(r->playerBufferQueue, bqPlayerCallback, r);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
    ...
    if (r->channelsRec) {
        ...
        SLDataLocator_IODevice loc_devI = {SL_DATALOCATOR_IODEVICE,
        ...
        result = (*r->engineEngine)
                     ->CreateAudioRecorder(r->engineEngine, &r->recorderObject,
        ...
        result =
            (*r->recorderObject)->Realize(r->recorderObject, SL_BOOLEAN_FALSE);
        if (SL_RESULT_SUCCESS != result) {
        ...
        result = (*r->recorderObject)
                     ->GetInterface(r->recorderObject, SL_IID_RECORD,
                                    &r->recorderRecord);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        ...
        // register callback on the buffer queue
        result = (*r->recorderBufferQueue)
                     ->RegisterCallback(r->recorderBufferQueue,
                                        bqRecorderCallback, r);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
    }
    ...
    if (r->playerPlay) {
        result =
            (*r->playerPlay)->SetPlayState(r->playerPlay, SL_PLAYSTATE_STOPPED);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        ...
        result = (*r->playerBufferQueue)
                     ->Enqueue(r->playerBufferQueue, r->playerBuffer,
                               r->playerBufferSizeBytes);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        result =
            (*r->playerPlay)->SetPlayState(r->playerPlay, SL_PLAYSTATE_PLAYING);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
    }
    ...
    if (r->recorderRecord) {
        result =
            (*r->recorderRecord)
                ->SetRecordState(r->recorderRecord, SL_RECORDSTATE_STOPPED);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
        ...
        // (for streaming recording, we would enqueue at least 2 empty buffers
        // to start things off)
        result = (*r->recorderBufferQueue)
                     ->Enqueue(r->recorderBufferQueue, r->recorderBuffer,
                               r->recorderBufferSizeBytes);
        ...
        result =
            (*r->recorderRecord)
                ->SetRecordState(r->recorderRecord, SL_RECORDSTATE_RECORDING);
        assert(SL_RESULT_SUCCESS == result);
        (void) result;
    }
...
int CNFAStateAndroid(void *v)
{
    ...
    return ((soundobject->recorderObject) ? 1 : 0) |
           ((soundobject->playerObject) ? 2 : 0);
}
...
void *InitCNFAAndroid(CNFACBType cb,
                      const char *your_name,
                      int reqSPSPlay,
                      int reqSPSRec,
                      int reqChannelsPlay,
                      ...
                      void *opaque)
{
    struct CNFADriverAndroid *r =
        (struct CNFADriverAndroid *) malloc(sizeof(struct CNFADriverAndroid));
    memset(r, 0, sizeof(*r));
...
// Tricky: On Android, this can't actually run before main.  Have to manually
// execute it.
...
#elif CNFA_SUN
// Copyright 2015-2020 <>< Charles Lohr under the MIT/x11, NewBSD or ColorChord
// License.  You choose.
...
void CloseCNFASun(void *v)
{
    struct CNFADriverSun *r = (struct CNFADriverSun *) v;
    if (r) {
        if (r->playback_handle != -1)
            close(r->playback_handle);
        if (r->record_handle != -1)
            close(r->record_handle);

        if (r->threadPlay)
            OGJoinThread(r->threadPlay);
        if (r->threadRec)
            OGJoinThread(r->threadRec);
...
void *RecThread(void *v)
{
    ...
    do {
        ...
        r->recording = 1;
        r->callback((struct CNFADriver *) r, NULL, r->samplesRec, 0,
                    (nread / 2) / r->channelsRec);
    } while (1);
    r->recording = 0;
...
void *PlayThread(void *v)
{
    ...
    while (err >= 0) {
        r->callback((struct CNFADriver *) r, r->samplesPlay, NULL, r->bufsize,
                    0);
        err = write(r->playback_handle, r->samplesPlay, nbytes);
        r->playing = 1;
    }
...
static struct CNFADriverSun *InitSun(struct CNFADriverSun *r)
{
    ...
    printf(
        "CNFA Sun Init -> devPlay: %s, channelsPlay: %d, spsPlay: %d, devRec: "
        "%s, channelsRec: %d, spsRec: %d\n",
        devPlay, r->channelsPlay, r->spsPlay, devRec, r->channelsRec,
        r->spsRec);

    if (r->channelsPlay && r->channelsRec && strcmp(devPlay, devRec) == 0) {
        if ((r->playback_handle = r->record_handle = open(devPlay, O_RDWR)) <
            0) {
    ...
    if (r->playback_handle) {
        ...
        if ((r->samplesPlay = calloc(2 * r->channelsPlay, r->bufsize)) ==
            NULL) {
            goto fail;
        }
    }
    ...
    printf(
        "CNFA Sun Init Out -> channelsPlay: %d, spsPlay: %d, channelsRec: %d, "
        "spsRec: %d\n",
...
void *InitSunDriver(CNFACBType cb,
                    const char *your_name,
                    int reqSPSPlay,
                    int reqSPSRec,
                    int reqChannelsPlay,
                    ...
                    void *opaque)
{
    struct CNFADriverSun *r =
        (struct CNFADriverSun *) malloc(sizeof(struct CNFADriverSun));
...
void CloseCNFAAlsa(void *v)
{
    ...
    if (r) {
        if (r->playback_handle)
            snd_pcm_close(r->playback_handle);
        if (r->record_handle)
...
static int SetHWParams(snd_pcm_t *handle,
                       int *samplerate,
                       short *channels,
                       snd_pcm_uframes_t *bufsize,
                       struct CNFADriverAlsa *a)
{
    int err;
    int bufs;
    int dir;
    ...
    if ((err = snd_pcm_hw_params_set_rate_near(
             handle, hw_params, (unsigned int *) samplerate, 0)) < 0) {
    ...
    bufs = *bufsize * 3;
    if ((err = snd_pcm_hw_params_set_buffer_size(handle, hw_params, bufs)) <
        0) {
        fprintf(stderr,
                "cannot set snd_pcm_hw_params_set_buffer_size size. (%s)\n",
                snd_strerror(err));
        goto fail;
    }
...
static int SetSWParams(struct CNFADriverAlsa *d, snd_pcm_t *handle, int isrec)
{
    ...
    if (!isrec) {
        ...
        if ((err = snd_pcm_sw_params_current(handle, sw_params)) < 0) {
            fprintf(
                stderr,
                "cannot initialize software parameters structure (%s) (%p)\n",
        ...
        // if ((err = snd_pcm_sw_params_set_stop_threshold(handle, sw_params,
        // 512 )) < 0) { 	fprintf (stderr, "cannot set minimum available count
        //(%s)\n", 		 snd_strerror (err)); 	goto fail;
        // }
        if ((err = snd_pcm_sw_params_set_start_threshold(
                 handle, sw_params, buffer_size - period_size)) < 0) {
            fprintf(stderr, "cannot set minimum available count (%s)\n",
                    snd_strerror(err));
            goto fail;
        }
...
static struct CNFADriverAlsa *InitALSA(struct CNFADriverAlsa *r)
{
    printf("CNFA Alsa Init %p %p  (%d %d) %d %d\n", r->playback_handle,
           r->record_handle, r->spsPlay, r->spsRec, r->channelsPlay,
           r->channelsRec);

    int err;
    if (r->channelsPlay) {
        if ((err = snd_pcm_open(&r->playback_handle,
                                r->devPlay ? r->devPlay : "default",
    ...
    if (r->channelsRec) {
        if ((err = snd_pcm_open(&r->record_handle,
                                r->devRec ? r->devRec : "default",
    ...
    if (r->playback_handle) {
        if (SetHWParams(r->playback_handle, &r->spsPlay, &r->channelsPlay,
                        &r->bufsize, r) < 0)
            goto fail;
    ...
    if (r->record_handle) {
        if (SetHWParams(r->record_handle, &r->spsRec, &r->channelsRec,
                        &r->bufsize, r) < 0)
            goto fail;
...
void *InitALSADriver(CNFACBType cb,
                     const char *your_name,
                     int reqSPSPlay,
                     int reqSPSRec,
                     int reqChannelsPlay,
                     ...
                     void *opaque)
{
    struct CNFADriverAlsa *r =
        (struct CNFADriverAlsa *) malloc(sizeof(struct CNFADriverAlsa));
...
void CloseCNFAPulse(void *v)
{
    ...
    if (r) {
        ...
        if (r->sourceNamePlay)
            free(r->sourceNamePlay);
        if (r->sourceNameRec)
            free(r->sourceNameRec);
        free(r);
    }
}
...
static void stream_request_cb(pa_stream *s, size_t length, void *userdata)
{
    ...
    r->callback((struct CNFADriver *) r, bufp, 0,
                length / (sizeof(short) * r->channelsPlay), 0);
...
static void stream_record_cb(pa_stream *s, size_t length, void *userdata)
{
    ...
    r->callback((struct CNFADriver *) r, 0, buffer, 0,
                length / (sizeof(short) * r->channelsRec));
    pa_xfree(buffer);
}
...
void *InitCNFAPulse(CNFACBType cb,
                    const char *your_name,
                    int reqSPSPlay,
                    int reqSPSRec,
                    int reqChannelsPlay,
                    ...
                    void *opaque)
{
    ...
    struct CNFADriverPulse *r =
        (struct CNFADriverPulse *) malloc(sizeof(struct CNFADriverPulse));
    ...
    printf("Pulse: from: [O/I] %s/%s (%s) / (%d,%d)x(%d,%d) (%d)\n",
           r->sourceNamePlay, r->sourceNameRec, title, r->spsPlay, r->spsRec,
           r->channelsPlay, r->channelsRec, r->buffer);
    ...
    if (r->channelsPlay) {
        ...
        /* The absolute maximum number of bytes that can be stored in the
         * buffer. If this value is exceeded then data will be lost. It is
         * recommended to pass (uint32_t) -1 here which will cause the server to
         * fill in the maximum possible value.*/
        bufattr.maxlength = bufBytesPlay * 3;

        /* The target fill level of the playback buffer. The server will only
         * send requests for more data as long as the buffer has less than this
         * number of bytes of data. If you pass (uint32_t) -1 (which is
         * recommended) here the server will choose the longest target buffer
         * fill level possible to minimize the number of necessary wakeups and
         * maximize drop-out safety. This can exceed 2s of buffering. For
         * low-latency applications or applications where latency matters you
         * should pass a proper value here. */
        bufattr.tlength = bufBytesPlay * 3;

        /* Number of bytes that need to be in the buffer before playback will
         * commence. Start of playback can be forced using pa_stream_trigger()
         * even though the prebuffer size hasn't been reached. If a buffer
         * underrun occurs, this prebuffering will be again enabled. If the
         * playback shall never stop in case of a buffer underrun, this value
         * should be set to 0. In that case the read index of the output buffer
         * overtakes the write index, and hence the fill level of the buffer is
         * negative. If you pass (uint32_t) -1 here (which is recommended) the
         * server will choose the same value as tlength here. */
        bufattr.prebuf = (uint32_t) -1;

        /* Minimum free number of the bytes in the playback buffer before the
         * server will request more data. It is recommended to fill in
         * (uint32_t) -1 here. This value influences how much time the sound
         * server has to move data from the per-stream server-side playback
         * buffer to the hardware playback buffer. */
        bufattr.minreq = (uint32_t) -1;

        /* Maximum number of bytes that the server will push in one chunk for
         * record streams. If you pass (uint32_t) -1 (which is recommended)
         * here, the server will choose the longest fragment setting possible to
         * minimize the number of necessary wakeups and maximize drop-out
         * safety. This can exceed 2s of buffering. For low-latency applications
         * or applications where latency matters you should pass a proper value
         * here. */
        bufattr.fragsize = (uint32_t) -1;

        int ret = pa_stream_connect_playback(
            r->play, r->sourceNamePlay, &bufattr,
            // PA_STREAM_INTERPOLATE_TIMING
            // |PA_STREAM_ADJUST_LATENCY //Some servers don't like the
            // adjust_latency flag. |PA_STREAM_AUTO_TIMING_UPDATE, NULL, NULL);
            PA_STREAM_NOFLAGS, NULL, NULL);
        if (ret < 0) {
            fprintf(stderr,
                    __FILE__
                    ": (PLAY) pa_stream_connect_playback() failed: %s\n",
                    pa_strerror(ret));
            goto fail;
        }
    }
    ...
    if (r->channelsRec) {
        ...
        bufattr.maxlength =
            (uint32_t) -1;  //(uint32_t)-1; //XXX: Todo, should this be low?
        bufattr.minreq = bufBytesRec;
        bufattr.prebuf = (uint32_t) -1;
        bufattr.tlength = bufBytesRec * 3;
        int ret = pa_stream_connect_record(
            r->rec, r->sourceNameRec, &bufattr,
            // PA_STREAM_INTERPOLATE_TIMING
            PA_STREAM_ADJUST_LATENCY  // Some servers don't like the
                                      // adjust_latency flag.
        ...
        if (ret < 0) {
            fprintf(stderr,
                    __FILE__
                    ": (REC) pa_stream_connect_playback() failed: %s\n",
                    pa_strerror(ret));
            goto fail;
        }
    }
