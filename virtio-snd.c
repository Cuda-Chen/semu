    ...
    {                                                                         \
        ...
        list_for_each_entry (node, &q, q) {                                   \
            ...
            (/* dequeue frames */                                             \
             ...
             if (bad_msg_err != 0) goto early_continue;                       \
        ...
        if (bad_msg_err != 0)                                                 \
            goto finally;                                                     \
        IIF(WRITE)                                                            \
        (/* dequeue frames */ /* Send signal if and only if we consume : 1) a \
                                 period size of frames 2) the end of the      \
                                 stream. */                                   \
         virtio_snd_prop_t *props = &vsnd_props[stream_id];                   \
         props->lock.buf_ev_notity--;                                         \
